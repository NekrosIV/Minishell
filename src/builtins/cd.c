...
/*   Updated: 2024/06/02 19:00:14 by pscala           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	replace_in_env(t_var *var, char *env)
{
	...
	pwd = getcwd(NULL, 0);
	if (!pwd)
		return (free_error(NULL, "pwd: " E_DIR_ERROR E_GETCWD_ERROR, strerror(errno),
			-1));
...
{
	if (!path)
		return (free_error(NULL, "cd: ", "\"HOME\" is not defined",
			-1), 1);
...
int	cd(char **cmd, t_var *var)
{
	...
	{
			free_error(NULL, "cd: ", "too many arguments",
			-1);
		return (1);
	}
	return (0);
